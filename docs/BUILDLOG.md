...
- **Live path built.** `fieldguard/live.py` runs one document through the same
  ...
  every stage; `fieldguard/server.py` (stdlib `http.server`, no deps, as ever)
  streams those events as SSE to `web/live.html`. Paste a document, pick a local
  model, watch the mechanism execute. Confirmed against Ollama end-to-end:
  SROIE/qwen2.5:3b 2 calls vs 6 (67% saved, nothing disputed);
  Kleister/qwen2.5:1.5b flagged `party` at score 1.00, one blind arbiter call,
  resolution `split-kept`, 3 calls vs 6.
- The demo cannot drift into theatre: `tests/test_live.py` pins the streamed
  final record, the call count and the baseline count to `pipeline.run`'s on
  the same input. If the live path ever diverges from the measured path, the
  suite fails. 47 -> 51 tests.
- Each stage carries its own explanation and, where it matters, the
  counterfactual (what plain schema-enforcement would have shipped). The
  verdict panel puts `baseline` and `record` side by side, scored against gold
  when a shipped sample is selected — the claim is shown, not asserted.
- **Correction, raised by the user: the 1.000 in the README's synthetic-invoice
  table was never a result and should not have been presented as one.** Not
  overfitting in the strict sense (nothing is trained here), but the failure is
  adjacent and just as disqualifying: those 8 documents are generated by this
  repo, qwen2.5:3b reads all 40 fields correctly under constraint, so
  `corruption_rate` is **0.000** — there was nothing to repair, and 1.000 ->
  1.000 measures the dataset's lack of headroom, not the mechanism. A saturated
  benchmark cannot separate methods. Section relabelled "Smoke test, NOT a
  result", the zero-corruption row added, and the same caveat attached to
  `examples/demo.py`, whose 1.000 is true by construction (MockBackend is a
  perfect reader and we inject the corruptions ourselves).
- Standing lesson: report the headroom next to the score. An accuracy without a
  corruption rate beside it can hide the fact that the method was never tested.
  The load-bearing evidence stays SROIE (0.820 -> 0.855, -61% calls) and
  Kleister (0.550 -> 0.566; -> 0.647 with grounding repair at zero extra calls).
