"""Accuracy vs verification-cost figures: three models per benchmark.

Reads results/<prefix>_*_n<n>_t0.5.json, emits docs/tradeoff_<bench>.svg
...
BENCHMARKS = (  # (results prefix, n, title, x_max, x_step, out file)
    ("sroie_50_desc", 50, "SROIE receipts (50 docs / 200 fields)", 330, 50,
     "tradeoff_sroie.svg"),
...
def render(prefix: str, n: int, title: str, x_max: int, x_step: int,
           outfile: str) -> None:
    ...
    s = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" '
         ...
         f'Accuracy vs verification cost — {title}</text>',
    ...
    for calls in range(0, x_max + 1, x_step):
    ...
    s.append(f'<text x="{ML+PW/2}" y="{H-10}" font-size="12" fill="{INK2}" '
             f'text-anchor="middle">LLM calls ({n} docs)</text>')
    ...
    xf = sx(full_cost)
    s.append(f'<line x1="{xf:.1f}" y1="{MT}" x2="{xf:.1f}" y2="{MT+PH}" '
             f'stroke="{AXIS}" stroke-width="1" stroke-dasharray="4 4"/>')
    s.append(f'<text x="{xf-6:.1f}" y="{MT+12}" font-size="11" fill="{MUTED}" '
             f'text-anchor="end">verify-everything cost</text>')

    for rep, name, color, side in points:
        calls, acc = rep["llm_calls"], rep["final_acc"]
        x, y = sx(calls), sy(acc)
        saved = 1 - calls / rep["full_verify_calls"]
    ...
    s.append("</svg>")
    out = ROOT / "docs" / outfile
    out.write_text("\n".join(s))
    print(f"figure -> {out}")


def main() -> None:
    for prefix, n, title, x_max, x_step, outfile in BENCHMARKS:
        render(prefix, n, title, x_max, x_step, outfile)
